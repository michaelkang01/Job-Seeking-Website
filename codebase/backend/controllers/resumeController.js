...
const uploadResume = (bucketID) => multer({
    storage: multerS3({
        ...
        key: function(req, file, cb){
            ...
            cb(null, `resume-${Date.now()}-${crypto.randomBytes(64).toString('hex')}.pdf`);
        },
    }),
});
...
export function setResume(req, res, next) {
    ...
    uploadSingle(req, res, async(err) => {
        ...
        db.jobseekerprofiles.updateOne({email: res.locals.authData.email}, {$set:{resumeUrl: req.file.location}}); // update the resumeUrl field in db
