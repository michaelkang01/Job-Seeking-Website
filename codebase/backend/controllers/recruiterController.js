...
const showApplicationsRoute = async (router, ApplicationsModel, RecruiterProfileModel, JobseekerModel, UserModel, Pitchmodel) => {
    ...
    router.get('/recruiter/applications', [verifyUser], (req, res) => {
        ...
        if (role !== 'Recruiter') {
            return res.status(401).json({
                success: false,
                message: 'You are not a recruiter; therefore, you cannot view applicants\' applications'
            })
        }
        ...
        RecruiterProfileModel.findOne({ user: id }).then(async (recruiter) => {
            ...
            for (const listing_id of recruiter.jobsPosted) {
                const applications = await ApplicationsModel.find({ listing_id: listing_id });
                console.log(applications);
                for (const application of applications) {
                    const jobSeeker = await JobseekerModel.findOne({ user: application.user_id });
...
const showApplicationRoute = async (router, ApplicationsModel, RecruiterProfileModel, JobseekerModel, UserModel, Pitchmodel) => {
    ...
    router.get('/recruiter/application', [verifyUser], (req, res) => {
        ...
        ApplicationsModel.findOne({ _id: applicationId }).then(async (application) => {
            if (!application) {
                return res.status(500).json({
                    success: false,
